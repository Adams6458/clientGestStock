    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(http("Get all transactions").get("/api/transactions").headers(headers_http_authenticated).check(status().is(200)))
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created transaction").get("${new_transaction_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created transaction").delete("${new_transaction_url}").headers(headers_http_authenticated))
                .pause(10)
        );
